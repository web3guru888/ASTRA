...
\item \textbf{Merger rate evolution}: Galaxy merger signatures correlate with redshift within the sample range.
   ...
   \item Literature: Merger rate evolution is well-established (Lotz et al.~2011)\cite{lotz2011}
...
\begin{enumerate}
\item \textbf{Evidence computation}: Marginal likelihood estimation using bridge sampling (Meng \& Wong 1996)\cite{meng1996}
...
\end{split}
\end{equation}
where \text{statistical\_strength} = $1 - p$-value (lower p = higher novelty), \text{unexpectedness} = 1.0 for patterns found in blind mode, \text{interpretability} measures whether a physical mechanism can be identified, and \text{consistency} measures internal coherence. Literature comparison is disabled during knowledge isolation mode.
...
\item Generate competing hypotheses and rank them by multiple criteria
