...
Code and documentation: \url{https://github.com/web3guru888/ASTRA} \cite{astra_github}.
\end{abstract}
...
The complete system is publicly available at: \url{https://github.com/web3guru888/ASTRA} \cite{astra_github}.
...
The qualitative preference for power-law/logarithmic models over the linear model is robust; the large reported Bayes factor ($33{,}000\times$) should be understood as indicating strong preference, though the naive harmonic mean estimator is known to overestimate absolute evidences (the true Bayes factor for power law vs linear may be $\sim 10$--$100\times$ rather than $33{,}000\times$; see \citealt{kass1995} for a discussion of estimation sensitivity). The broken power-law result requires more caution: despite reporting a BIC-penalized Bayes factor of $123{,}000\times$ against the broken power law, this model achieves the highest $R^2$ (0.959) with only two additional parameters over the power-law ($n = 24$), and BIC-based cross-checks suggest the broken power law may not be disfavoured once estimation bias is corrected. We therefore restrict the strong Occam's razor interpretation to the power-law versus linear comparison, where the directional preference is unambiguous.
...
\label{eq:bias}
\end{equation}
agreeing with the analytic prediction $k(n=7) = 2$ to within 4 per cent. The resolution
...
\begin{figure}
\centering
\includegraphics[width=0.95\columnwidth]{figures/test07_contrast_vs_beta.pdf}
...
\begin{figure}
\centering
\includegraphics[width=0.95\columnwidth]{figures/test07_stability_boundary.pdf}
...
Belt filaments at $d < 500$~pc and is not applicable to W3 at 1.95~kpc; recent work by
\citet{rivera2013} provides the appropriate Herschel characterization of W3 specifically.
...
\textbf{Compared to Causal ML Approaches}: Recent work has applied causal machine learning directly to astrophysical problems. \citet{mucesh2024} applied doubly-debiased machine learning and causal inference to IllustrisTNG hydrodynamical simulations to disentangle the contributions of environment and stellar mass to star formation quenching. \citet{desmond2026} applied the Fast Causal Inference (FCI) algorithm to 450,000 galaxies to discover causal relationships between morphological and photometric properties. \citet{jin2025} used Bayesian causal discovery to constrain the coevolution of supermassive black holes and their host galaxies, demonstrating causal methods on observational data with realistic confounders. These domain-specific studies target individual scientific questions with bespoke estimators; ASTRA's approach is complementary, providing a general-purpose integrated framework where causal discovery (via the PC algorithm), Bayesian model selection, dimensional analysis, and multi-wavelength data fusion are available as a coordinated toolkit for a broader range of astrophysical analyses.
