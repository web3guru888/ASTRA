...
computing (HPC) facility. ASTRA co-ordinated over 700 Athena++ \citep{stone2020} magnetohydrodynamic
(MHD) simulations across four interlinked campaigns to validate the magnetic Jeans fragmentation
...
line-mass, and boundary condition variations.
...
ASTRA co-ordinated four interlinked campaigns on the astra-climate GCE server (224-vCPU AMD
...
To assess robustness and generality, ASTRA co-ordinated an additional 89 Athena++ MHD
simulations in three categories: near-critical fragmentation (36 sims), field geometry
sensitivity (40 sims), and numerical robustness checks (13 sims). All 89 simulations
fragmented (100\% fragmentation rate), confirming the universality of gravitational
fragmentation in thermally supercritical filaments across a wide range of conditions.
...
the system formulated the scientific question, selected simulation parameters, managed over 700
...
By co-ordinating over 700 Athena++ simulations on a remote HPC facility, ASTRA validated the
...
numerical predictions. A subsequent peer review robustness campaign of 89 simulations
confirmed that the main DTC results are robust to field geometry (non-longitudinal angles
delay but do not prevent fragmentation), near-critical line-mass ($f = 1.00$--$1.05$
filaments all fragment, albeit on longer timescales of $0.88$--$1.63\,t_J$), domain size,
boundary conditions, and turbulence prescription. This demonstrates that ASTRA can function
as an autonomous simulation manager capable of designing and executing multi-campaign
robustness validation without human intervention.
...
\textbf{Test 7: Autonomous MHD Simulation Campaign} (over 700 Athena++ simulations)
\begin{description}
