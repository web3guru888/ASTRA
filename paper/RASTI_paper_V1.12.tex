...
\begin{enumerate}
\item \textbf{Evidence computation}: Marginal likelihood estimation via Monte Carlo sampling \citep{meng1996}
...
\caption{Bayesian model selection for 24 Herschel filaments. Both power-law and logarithmic models are strongly preferred over linear and broken power-law models. The power-law model is theoretically motivated by the virial theorem. The measured power-law exponent $\alpha = 0.49 \pm 0.03$ is consistent with the virial prediction of $0.5$ at $0.33\sigma$ (slope tension). Separately, the virial \emph{amplitude} shows a 2.7$\sigma$ deficit ($A_{\mathrm{meas}} = 0.0812$ vs $A_{\mathrm{theory}} \approx 0.0928$~km\,s$^{-1}$\,(M$_\odot$\,pc$^{-1}$)$^{-0.5}$), suggesting possible distance systematics or non-virial support mechanisms.}
\label{fig:bayesian}
\end{figure*}
...
Characterization of the 150 outliers using their positions in the star-forming main sequence, colour--mass, and metallicity--mass planes revealed four distinct sub-populations:
\begin{itemize}
\item \textbf{Starbursts} (47 galaxies): SFR significantly elevated ($> 0.6$~dex) above the main sequence; blue colours ($g-r < 0.55$). Consistent with merger-induced or AGN-driven star formation.
\item \textbf{Post-starbursts} (35 galaxies): Below the main sequence ($> 0.4$~dex) but with blue colours ($g-r < 0.60$) and sSFR $> 10^{-11.5}$~yr$^{-1}$, consistent with photometric E+A signatures. Spectroscopic follow-up is required to confirm H$\alpha$ absorption and Balmer line features.
\item \textbf{Transitioning} (40 galaxies): Moderate SFR suppression in the green valley; intermediate colours and sSFR, consistent with ongoing quenching.
\item \textbf{Unclassified below-MS} (28 galaxies): Deeply quenched (low sSFR) but lacking the photometric signatures of post-starbursts; likely passively evolving objects.
\end{itemize}
The 47 + 35 + 40 + 28 = 150 count accounts for all identified outliers.
...
ASTRA ranked the 150 outliers by discovery potential using a multi-criterion triage. The four categories identified in Section~8.2.1 were prioritised as follows:

\begin{enumerate}
\item \textbf{Starbursts (47)}: Highest priority for follow-up as potential merger-induced systems or AGN hosts with elevated far-infrared emission. Ranked by degree of main-sequence offset.
\item \textbf{Post-starburst candidates (35)}: High priority for spectroscopic confirmation; photometric blue colours and suppressed SFR are consistent with E+A signatures but require H$\alpha$ absorption and Balmer line measurement to confirm.
\item \textbf{Transitioning galaxies (40)}: Moderate priority; intermediate sSFR places them in the green valley, a transient phase that statistical samples characterise well but individual-object follow-up can constrain quenching timescales.
\item \textbf{Unclassified below-MS (28)}: Lower priority; likely passively evolving red-sequence objects, though deeper photometry could reveal residual star formation.
\end{enumerate}

The 47 starbursts and 35 post-starburst candidates were recommended as the highest priority targets for spectroscopic follow-up.
...
\caption{\textbf{Test Case 6: Star-Forming Main Sequence with Discovery Analysis}. Panel A shows the main sequence (log SFR vs log stellar mass) for 3,000 synthetic galaxies, colour-coded by specific SFR. The dashed line shows ASTRA's recovered relation (slope = 0.59). The 150 outlier galaxies identified by isolation-forest analysis are colour-coded by class: starbursts (coral, $n=47$), post-starbursts (cyan, $n=35$), transitioning (amber, $n=40$), and unclassified (violet, $n=28$). Panel B shows the bimodal specific SFR distribution. Panel C shows the colour--mass diagram with outliers highlighted.}
\label{fig:test06a}
\end{figure*}
...
\caption{\textbf{Test Case 6: Multi-Dimensional Outlier Detection}. Panel A shows the specific SFR distribution of 3,000 galaxies (grey) with the four outlier classes overlaid (colour-coded as in Figure~\ref{fig:test06a}); the dashed line marks the star-forming/quenched boundary at sSFR $= 10^{-11}$ yr$^{-1}$. Panel B shows the colour--mass diagram; the inset pie chart summarises the four-category breakdown: 47 starbursts + 35 post-starbursts + 40 transitioning + 28 unclassified = 150 total ($5.0$ per\,cent of the sample), all identified by isolation-forest analysis in five-dimensional parameter space (mass, SFR, metallicity, colour, sSFR).}
\label{fig:test06c}
\end{figure*}
...
\item Detected 2.7$\sigma$ deficit in virial \emph{amplitude} ($A_{\mathrm{meas}}/A_{\mathrm{theory}} = 0.88$); virial \emph{slope} $\alpha = 0.49 \pm 0.03$ is consistent with the predicted exponent $0.5$ at $0.33\sigma$
...
\item Classified all 150 outliers into four categories: 47 starbursts (above MS, elevated SFR), 35 post-starburst candidates (below MS, blue colours), 40 transitioning galaxies (green valley), and 28 unclassified below-MS objects
