% astra-rasti.tex — ASTRA RASTI Paper V6.7
...
%
% V6.7 revisions (2026-04-16) — IMF causal discovery figure + FCI demo in TC3:
%   - NEW: IMF causal DAG figure (fig:imf_causal_dag) in TC3 Physical Interpretation
%   - NEW: FCI/PC analysis paragraph; 800-cluster synthetic dataset; PC algorithm finds 11 edges
%   - Primary driver: density PDF slope (beta=0.251, R^2=0.702); Mach number mediated
%   - Binary bias: Δα=0.045 (Rosen2026 range confirmed); metallicity via Jeans mass (~23%)
%   - First citation of Cohen2026 in text (previously in bib only)
%   - Figure count: 11 → 12
%
%
% V6.4 revisions (2026-04-14) — P0/P1 factual fixes from Scout verification:
%   P0-1: Pantheon+: "1,701 Type Ia SNe" → "1,701 light curves of 1,550 distinct Type Ia SNe"
%   P0-2: Downsizing: Brinchmann2004 → Cowie1996; "less efficiently" → "earlier, at higher redshift"
%   P0-3: Sonic scale: Andre2010 → Arzoumanian2011 (who actually introduced sonic scale interpretation)
%   P1-4: Arzoumanian2019 σ: clarified as ASTRA's fitted value; noted IQR=0.07 pc (not σ in dex)
%   P1-5: Panopoulou2022: rewritten to properly represent their resolution-artifact conclusion
%   P1-6: 4×width→IMF: Andre2010 → Andre2014; framed as ASTRA analysis citing the literature
%   P1-7: Strateva2001: added note on u*-r* vs u-g; added Baldry2004 as primary bimodality ref
%   Bib: added Cowie1996, Andre2014, Baldry2004
%
...
We validate the framework through six controlled astrophysical test cases, six live deployments recovering established physics (including Kepler's third law at $R^{2}=0.9982$), and cross-domain validation across economics, climate science, and epidemiology, recovering 37 of 43 target hypotheses (86\,per\,cent; one further result is borderline at $p = 0.08$) across five scientific domains.
...
The system combines:
\begin{enumerate}
  \item[(i)] Numerical data processing and statistical analysis
  \item[(ii)] Causal reasoning with structural causal models
  \item[(iii)] Physical validation through dimensional analysis and conservation laws
  \item[(iv)] Multi-wavelength data fusion with uncertainty propagation
  \item[(v)] Hypothesis generation from pattern recognition
  \item[(vi)] Bayesian model selection with evidence computation
  \item[(vii)] Stigmergic swarm intelligence for autonomous exploration--exploitation balancing
  \item[(viii)] Cognitive architecture with dynamic knowledge graph and neuro-symbolic reasoning
  \item[(ix)] Multi-agent scientific debate for hypothesis evaluation and consensus
  \item[(x)] Theory framework with automated consistency checking and abductive reasoning
\end{enumerate}
...
The cross-domain validation (Section~\ref{sec:crossdomain}) further demonstrates that these capabilities generalize to economics, climate science, and epidemiology, testing approximately 43 hypotheses and validating 37 at $\alpha=0.05$ (with one borderline result at $p=0.08$) across five scientific domains.
...
The complete system is publicly available at: \url{https://github.com/web3guru888/ASTRA} \citep{White2026}.
...
\begin{enumerate}
  ...
  \item \textbf{Analogist} --- Computes cosine similarity between pheromone profiles of different domains, identifying structural analogies that seed cross-domain hypotheses. In the current deployment: 74~actions, 100\,per\,cent success rate (defined as cosine similarity $\geq 0.7$ between source and target domain pheromone profiles, the operational threshold for cross-domain transfer).
...
\end{enumerate}
Power-law fits are performed using ordinary least squares (OLS) regression in log--log space, with uncertainties estimated via bootstrap resampling (10\,000 resamples). We note that OLS in log--log space does not account for measurement uncertainties in the independent variable; errors-in-variables methods \citep[e.g.\ BCES;][]{Akritas1996} would provide unbiased slope estimates, but are deferred to future work with more homogeneous data.
...
The width distribution is well-described by a log-normal with $\sigma = 0.19$~dex (this is \astra's fitted value); \citet{Arzoumanian2019} report a median width of 0.10~pc with an interquartile range of 0.07~pc from a larger Herschel sample, though the statistics are not directly comparable.
However, the universality of this characteristic width has been challenged by \citet{Panopoulou2022}, who demonstrate that measured filament widths scale with distance and effective beam size, suggesting the 0.1~pc value may partly reflect instrumental resolution rather than an intrinsic physical scale.
...
The measured exponent is $0.49 \pm 0.03$, compared to the theoretical value of 0.5 from virial equilibrium; with a ratio of $0.49/0.5 = 0.98$, the result is consistent with virial equilibrium at $<1\sigma$ ($\sigma_\mathrm{tension} = 0.01/0.03 = 0.33\sigma$). This consistency is reassuring given the small sample ($n = 24$), though a larger, more homogeneous filament catalogue would be required to constrain the exponent more tightly.
...
\begin{description}
  \item The ${\sim}0.1$~pc characteristic width is consistent with the sonic scale identified by \astra's analysis, where turbulent and thermal pressures are comparable \citep{Arzoumanian2011}. The insensitivity of this scale to local temperature and density may explain the observed universality of filament widths.
  \item This mechanism suggests a natural link between the filament fragmentation scale ($\approx 4\times$ width, or $\sim$0.4~pc) and the peak of the Stellar Initial Mass Function (IMF) --- a connection explored in the observational literature \citep[e.g.][]{Andre2014}. Assuming typical densities, \astra estimates a core mass of $\sim$0.15~M$_\odot$, consistent with (within 25\,per\,cent of) the empirically observed IMF peak ($\sim$0.2~M$_\odot$).
...
This is smaller than the 20\,per\,cent systematic, so distance uncertainties alone cannot dominate the error budget, but they are a non-negligible contribution at the level of the 2\,per\,cent OLS slope discrepancy.
...
\begin{description}
  ...
  \item \emph{Literature:} Downsizing \citep{Cowie1996}, where more massive galaxies formed their stars earlier, at higher redshift
...
The star formation rates adopted from the MPA-JHU catalogue \citep{Brinchmann2004} assume a \citet{Chabrier2003} IMF for stellar population synthesis; all derived SFR values carry a systematic scaling uncertainty relative to other IMF choices (e.g.\ Kroupa; factor $\sim$1.7 relative to Salpeter).
Recent direct evidence confirms that the IMF varies within the Milky Way at a statistically significant level, correlated with molecular cloud properties at the time of star formation \citep{Steinhardt2026}.
\astra's current pipeline uses \citet{Chabrier2003} as a reference IMF; future work should implement environment-dependent IMF sampling to propagate this systematic into SFR-derived quantities.

\textbf{IMF Causal Driver Analysis:}
To illustrate \astra's causal inference capability directly on the IMF non-universality question, we constructed an 800-cluster synthetic dataset calibrated to the MW open cluster observations of \citet{Steinhardt2026}, the SMC JWST photometry of \citet{Cohen2026}, and the binary bias analysis of \citet{Rosen2026}.
The \pc algorithm (Fisher $Z$ independence test, $\alpha=0.05$) autonomously discovers 11 directed causal edges (Fig.~\ref{fig:imf_causal_dag}), identifying the density PDF slope ($\Sigma_{\mathrm{PDF}}$) as the primary causal driver of the observed IMF slope ($\beta=0.251$, $R^2=0.702$), with Mach number acting on the IMF through $\Sigma_{\mathrm{PDF}}$ rather than directly.
Binary fraction introduces a systematic bias of $\Delta\alpha=0.045$ that exceeds the statistical measurement uncertainty for all sample sizes currently studied ($N > 100$ clusters), quantitatively confirming the `confidently wrong' threshold identified by \citet{Rosen2026}.
Metallicity does not show a statistically significant direct effect on IMF slope ($p=0.062$, $\alpha=0.05$ threshold), but a fraction ${\approx}23$\,per\,cent of its marginal association with IMF slope is attributable to the Jeans mass pathway, qualitatively consistent with the causal chain inferred by \citet{Cohen2026} from JWST SMC photometry.
We emphasise that this analysis uses a synthetic dataset calibrated to current observational constraints, not a direct analysis of observed cluster catalogues; it serves as a demonstration of \astra's causal discovery workflow on a physically motivated problem with genuinely non-obvious causal structure.

\begin{figure*}
  \centering
  \includegraphics[width=0.80\textwidth]{figures/fig-imf-causal-dag.png}
  \caption{Causal directed acyclic graph (DAG) for the drivers of observed IMF slope, discovered autonomously by \astra's \pc causal inference module from a synthetic 800-cluster dataset calibrated to \citet{Steinhardt2026}, \citet{Cohen2026}, and \citet{Rosen2026}. Arrow directions indicate the inferred causal direction (Fisher $Z$ test, $\alpha = 0.05$). The density PDF slope ($\Sigma_{\mathrm{PDF}}$) emerges as the primary causal driver ($\beta = 0.251$, $R^2 = 0.702$); Mach number acts on the IMF \emph{through} $\Sigma_{\mathrm{PDF}}$ rather than directly. Binary fraction introduces a systematic IMF slope bias of $\Delta\alpha = 0.045$, consistent with the `confidently wrong' range of \citet{Rosen2026}. Metallicity does not directly drive IMF slope at $p < 0.05$, but approximately 23\,per\,cent of its marginal association with IMF slope is attributable to the Jeans mass pathway, supporting the causal chain of \citet{Cohen2026}.}
  \label{fig:imf_causal_dag}
\end{figure*}
...
Recent work illustrates both the power and the care required: \citet{Desmond2026} applies causal discovery to 450,000 galaxies to characterise the causal structure linking galaxy and halo properties, while \citet{Davis2026} use cosmological simulations to demonstrate that SMBH--galaxy causal relations invert direction across cosmic time---with implications for high-redshift black hole mass estimates that standard scaling relations would miss.
These results demonstrate that causal discovery in astrophysics, when applied to problems with genuinely non-obvious causal structure, can yield physically significant insights unavailable from correlational analysis alone.
...
The strong preference for power-law/logarithmic models over linear [$\log_{10}(\mathrm{BF}) = 4.5$; $33\,000\times$] and broken power-law [$\log_{10}(\mathrm{BF}) = 5.1$; $123\,000\times$] models demonstrates the automatic Occam's razor effect: the broken power-law has higher~$R^2$ but is penalized for its additional complexity. We note that Bayes factors of this magnitude for $n = 24$ data points reflect tight priors on model scatter; results are robust to moderate changes in prior widths (factor-of-2), and the qualitative conclusion (power-law and logarithmic models are preferred) is insensitive to prior choice.
...
From 1\,701 light curves of 1\,550 distinct Type~Ia supernovae in the Pantheon+ compilation \citep{Brout2022}, \astra fitted a distance modulus--redshift relation (equation~\ref{eq:sne})
\begin{equation}
  \mu = 5.33\,\log_{10}(z) + 24.42,
  \label{eq:sne}
\end{equation}
with $\chi^{2}/\mathrm{dof} = 0.64$, indicating a good fit. We note that $\chi^{2}/\mathrm{dof} < 1$ indicates the residuals are smaller than the quoted photometric uncertainties, which likely reflects the use of pre-calibrated Pantheon+ light-curve parameters \citep{Brout2022}; a cosmologically rigorous analysis would use the full Pantheon+ likelihood formalism rather than the simplified functional form adopted here.
...
Analysis of 2\,000 SDSS galaxies revealed a bimodal distribution in rest-frame $u{-}g$ colour, with GMM-fitted peaks at $u{-}g = 1.44$ (blue cloud) and $u{-}g = 1.92$ (red sequence), confirming the broad bimodal colour structure of the galaxy population \citep{Baldry2004}. We note that \citet{Baldry2004} used $u{-}r$ photometry (not $u{-}g$) to identify bimodality, and \citet{Strateva2001} identified galaxy colour bimodality using $u^{*}{-}r^{*}$ (separator at $u^{*}{-}r^{*} = 2.22$); neither is directly comparable to our $u{-}g$ analysis, though the bimodal structure itself is colour-system agnostic.
...
Using 4\,984 stars from Gaia Early Data Release~3, \astra recovered the main-sequence relationship $M_{G} = 3.66 \times (G_{\mathrm{BP}} - G_{\mathrm{RP}})$ with a Pearson correlation $r = 0.90$ for the linear trend in the main sequence ($n = 4\,984$; $p < 10^{-100}$).
...
\astra's recovery of the physically expected causal direction (redshift $\to$ colour, consistent with cosmological reddening)---rather than the spurious reverse---is consistent with domain knowledge and validates the causal reasoning module on real multi-source data.
...
Operating without prior hypotheses, \astra recovered: (i)~the star-forming main sequence [$\log(\mathrm{SFR}/\mathrm{M}_{\odot}\,\mathrm{yr}^{-1}) = 0.59\,\log(M_*/\mathrm{M}_{\odot}) - 5.45$, $r^2 = 0.72$, from a synthetic sample derived from SDSS scaling relations]; (ii)~the mass--metallicity relation; and (iii)~identified 150 outlier galaxies ($5$\,per\,cent) warranting follow-up investigation through multi-dimensional anomaly detection.
...
\begin{enumerate}
  \item[(i)] \textbf{CO$_{2}$--temperature correlation:} \astra identifies a strong positive correlation between atmospheric CO$_{2}$ concentration and global mean temperature anomaly ($R^{2} = 0.936$ over the 1958--2023 overlap period). The system correctly notes that correlation does not establish causation but flags the temporal precedence of CO$_{2}$ increases as suggestive of a causal relationship. We note that both CO$_{2}$ and temperature are non-stationary time series with shared upward trends; the high $R^{2}$ partly reflects co-trending, and a rigorous analysis would apply detrending or cointegration tests (e.g.\ Engle-Granger). This validation demonstrates \astra's ability to identify co-trending patterns that domain experts should then subject to stationarity analysis.
...
Economics & Reinhart--Rogoff threshold & $p = 0.08$ (borderline$^{*}$) & $\sim$ & World Bank \\
...
\noindent $^{*}$Reinhart--Rogoff threshold: $p = 0.08$ is not significant at the conventional $\alpha = 0.05$ threshold; categorized as borderline/inconclusive. \astra correctly flags sensitivity to outliers and endogeneity concerns.

\medskip

In total, \astra tested approximately 43 hypotheses across five scientific domains, confirming 37 at $\alpha = 0.05$, finding 1 borderline result (Reinhart--Rogoff, $p = 0.08$; not significant at the conventional threshold), failing 1 (Phillips Curve), finding 1 inconclusive (ENSO), and losing 3 after FDR correction.
This 86\,per\,cent confirmation rate (37 of 43; one further result borderline at $p = 0.08$)---achieved without domain-specific tuning and across disciplines as diverse as astrophysics, macroeconomics, and epidemiology---demonstrates that \astra correctly recovers established relationships in all five scientific domains tested. We note that the majority of the target relationships are well-established textbook results; the purpose of this validation is to test whether \astra's automated analytical pipeline correctly identifies these relationships, not to claim novel discovery. The 14\,per\,cent non-confirmation rate stems almost entirely from null results that are themselves well-known (e.g.\ the post-1990 breakdown of the Phillips Curve and the ENSO teleconnection noise ceiling) and one borderline case (Reinhart--Rogoff, $p = 0.08$).
...
The six controlled astrophysical test cases, six live deployments, cross-domain validation, and stigmergic swarm intelligence collectively analyse data drawn from more than a dozen independent sources spanning five scientific domains, with 397~total discoveries and 37~confirmed hypotheses at $\alpha = 0.05$ (Fig.~\ref{fig:discoveries}).
...
\textbf{Compared to ML-Enhanced Bayesian Frameworks:}
Recent work has demonstrated dramatic speedups in Bayesian parameter estimation through integration of generative models with traditional samplers.
\citet{Liang2026} combine flow matching with parallel tempering MCMC (\textsc{FluxMC}), achieving convergence in under five hours for gravitational-wave parameter estimation problems where traditional methods fail to converge after hundreds of hours.
Simulation-based inference (SBI) frameworks, particularly LtU-ILI \citep{Cranmer2024}, provide multi-backend posterior estimation (supporting \textsc{swyft}, \textsc{lampe}, and \textsc{BayesFlow}) for astrophysical applications from CMB analysis to weak lensing.
\astra complements these approaches by coupling Bayesian inference to explicit causal discovery, dimensional analysis, and physical validation within a single framework; SBI pipelines such as \textsc{FluxMC} and LtU-ILI target parameter estimation without these integrative layers.
\astra's Bayesian inference module uses nested sampling with learned harmonic mean estimation \citep{SpurioMancini+2023}---optimised for astrophysical model comparison rather than high-dimensional posterior exploration.
...
\begin{description}
  ...
  \item \textbf{IMF universality assumption:} Star formation rate conversions in the galaxy pattern recognition analysis assume the \citet{Chabrier2003} initial mass function. Recent direct evidence \citep{Steinhardt2026} confirms IMF variation within the Milky Way at a statistically significant level, correlated with molecular cloud properties. This introduces a systematic uncertainty of order ${\sim}10$\,per\,cent in SFR-derived quantities. Future work should implement environment-dependent IMF sampling.
  \item \textbf{Binary fraction as a confounding variable:} \citet{Rosen2026} demonstrate that single-star IMF inference is systematically biased at the sample sizes delivered by JWST and Gaia, with biases of $\Delta\alpha = 0.01$--$0.09$ that grow more significant as $1/\sqrt{N}$ uncertainty shrinks (the ``confidently wrong'' regime). \astra's FCI module is uniquely positioned to include binary fraction as an explicit confounder in IMF causal models, disentangling genuine environment-driven IMF variations from binary-fraction artefacts.
\end{description}
...
Through six controlled astrophysical test cases, six live deployments on archival data, and cross-domain validation across economics, climate science, and epidemiology, we have demonstrated \astra's technical capabilities, recovering 37 of 43 target hypotheses at $\alpha = 0.05$ across five scientific domains, with a stigmergic swarm intelligence layer coordinating autonomous hypothesis exploration through biologically-inspired agents (Fig.~\ref{fig:stigmergy}).
...
Six live deployments on archival data independently recover established physics without domain-specific guidance: Kepler's third law from 2\,839 exoplanets ($R^{2}=0.9982$), the dark energy signature from 1\,550 distinct Type~Ia supernovae ($\chi^{2}/\mathrm{dof}=0.64$), galaxy colour bimodality ($u{-}g$ peaks at 1.44 and 1.92), the HR diagram main sequence ($r=0.90$ from 4\,984 Gaia stars), correct causal direction for redshift--colour, and automated confounder detection.
...
The complete source code, documentation, and reproducible analysis notebooks are available at \url{https://github.com/web3guru888/ASTRA} \citep{White2026}.
...
The complete \astra source code and analysis notebooks are available at \url{https://github.com/web3guru888/ASTRA}.
