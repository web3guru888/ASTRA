...
The \pc algorithm (Fisher $Z$ independence test, $\alpha=0.05$) autonomously discovers 11 directed causal edges (Fig.~\ref{fig:imf_causal_dag}), identifying the density PDF slope ($\Sigma_{\rm PDF}$) as the primary causal driver of the observed IMF slope ($\beta=0.251$, $R^2=0.702$), with Mach number acting on the IMF through $\Sigma_{\rm PDF}$ rather than directly.
...
\begin{figure*}
  ...
  \caption{Causal directed acyclic graph (DAG) for the drivers of observed IMF slope, discovered autonomously by \astra's \pc causal inference module from a synthetic 800-cluster dataset calibrated to \citet{Steinhardt2026}, \citet{Cohen2026}, and \citet{Rosen2026}. Arrow directions indicate the inferred causal direction (Fisher $Z$ test, $\alpha = 0.05$). The density PDF slope ($\Sigma_{\rm PDF}$) emerges as the primary causal driver ($\beta = 0.251$, $R^2 = 0.702$); Mach number acts on the IMF \emph{through} $\Sigma_{\rm PDF}$ rather than directly. Binary fraction introduces a systematic IMF slope bias of $\Delta\alpha = 0.045$, consistent with the `confidently wrong' range of \citet{Rosen2026}. Metallicity does not directly drive IMF slope at $p < 0.05$, but approximately 23\,per\,cent of its marginal association with IMF slope is attributable to the Jeans mass pathway, supporting the causal chain of \citet{Cohen2026}.}
...
\astra's Bayesian inference module uses a complementary strategy---nested sampling with learned harmonic mean estimation \citep{SpurioMancini+2023}---optimised for astrophysical model comparison rather than high-dimensional posterior exploration.
Critically, \astra extends beyond parameter estimation by coupling Bayesian inference to causal discovery, dimensional analysis, and physical validation within a single framework; \textsc{FluxMC} and similar systems target a single domain without these integrative layers.
