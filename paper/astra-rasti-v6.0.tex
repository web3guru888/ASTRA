...
In evaluating competing mechanisms for the observed characteristic width, \astra autonomously tested four theoretical length scales. The system identified the **turbulent dissipation scale (sonic scale)** as the statistically preferred model:
\begin{itemize}
  \item \emph{Sonic Scale (best match):} Predicts $0.080 \pm 0.000$~pc, exhibiting minimal scatter across diverse ISM conditions and agreeing with the observed median value within 20~per~cent. High theoretical robustness based on the transition from supersonic to subsonic turbulence.
...
\begin{itemize}
  \item The ${\sim}0.1$~pc characteristic width is strongly supported by \astra's analysis as the sonic scale, where turbulent and thermal pressures are comparable \citep{Andre2010}. The insensitivity of this scale to local temperature and density explains the observed universality of filament widths.
  \item This mechanism naturally links the filament fragmentation scale ($\approx 4\times$ width, or $\sim$0.4~pc) to the peak of the Stellar Initial Mass Function (IMF). Assuming typical densities, \astra calculates a core mass of $\sim$0.15~M$_\odot$, in excellent agreement (25~per~cent difference) with the empirically observed IMF peak ($\sim$0.2~M$_\odot$).
