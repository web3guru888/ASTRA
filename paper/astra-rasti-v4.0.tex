% astra-rasti.tex — ASTRA RASTI Paper V4.0
...
We validate the framework through six controlled astrophysical test cases spanning nine data sources and more than 27\,430 data points, covering scaling relations, multi-wavelength cross-matching, pattern recognition, causal inference, Bayesian model selection, and a discovery-mode demonstration on synthetic data with known ground truth.
In the discovery test, \astra operates in ``knowledge isolation mode'' to correctly identify the causal drivers of star formation, distinguish column density as a proxy rather than direct cause, and generate testable predictions validated through intervention analysis.
Six live deployments on archival data independently recover established physics: Kepler's third law ($R^{2}=0.9982$), the dark energy signature from Type~Ia supernovae ($\chi^{2}/\mathrm{dof}=0.64$), and galaxy colour bimodality, among others.
Cross-domain validation on public data from economics, climate science, and epidemiology recovers approximately 42 hypotheses across five scientific domains---including Okun's Law, the Preston Curve, and CO$_{2}$--temperature coupling---while correctly identifying non-relationships and applying false discovery rate correction.
...
The dashboard is organized into four operational views.
...
\begin{figure*}
  \includegraphics[width=\textwidth]{figures/fig7-overview-final.png}
  \caption{The \astra Live Dashboard overview, showing the real-time monitoring interface during autonomous operation.  \emph{Left:} Activity stream logging statistical evaluations, FDR corrections, and engine decisions; below, the OODA decision engine displaying the current phase of the \textsc{Orient}--\textsc{Select}--\textsc{Investigate}--\textsc{Evaluate}--\textsc{Update} cycle. \emph{Centre:} Neural topology graph visualizing the hypothesis network. \emph{Right:} Data visualizations including the hypothesis funnel, confidence radar, $H_0$ distribution, discovery and error rates, and the autonomous decision log.}
  \label{fig:dashboard}
\end{figure*}

\begin{figure*}
  \includegraphics[width=\textwidth]{figures/fig8-safety-final.png}
  \caption{Safety monitoring dashboard.  \emph{Top left:} State-space mind trajectory projected onto the first two principal components (PC1: Confidence Variance, PC2: Domain Diversity), with concentric boundaries delineating nominal (green), caution (amber), and critical (red) operating regions; the system remains within the nominal basin after 50~cycles.  \emph{Top right:} Anomaly detection drift monitor tracking five metrics in real time, with colour-coded severity (Nominal, Warning, Critical).  \emph{Bottom:} Composite alignment stability (78.4\,per\,cent) scored across six dimensions: Scientific Rigour (82\,per\,cent), Reproducibility (92\,per\,cent), Epistemic Humility (83\,per\,cent), Novelty Pursuit (71\,per\,cent), Resource Efficiency (64\,per\,cent), and Domain Balance (48\,per\,cent).}
...
The six controlled astrophysical test cases, six live deployments, and cross-domain validation collectively analyse data drawn from more than a dozen independent sources spanning five scientific domains.
...
We have presented \astra (Autonomous System for Scientific Discovery in Astrophysics), an integrated framework for physics-aware scientific analysis that unifies dimensional analysis, causal reasoning, Bayesian model selection, and multi-wavelength data fusion within a single reproducible pipeline.
Through six controlled astrophysical test cases, six live deployments on archival data, and cross-domain validation across economics, climate science, and epidemiology, we have demonstrated \astra's technical capabilities across approximately 42 hypotheses in five scientific domains.
