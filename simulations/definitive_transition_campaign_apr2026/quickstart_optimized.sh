...
# Usage: bash quickstart_optimized.sh [--phase PHASE] [--resume]
#
...
check_dependencies() {
    ...
    if [ ! -f "$ATHENA_BINARY" ]; then
        ...
        log_error "Please update ATHENA_BINARY in quickstart_optimized.sh"
        exit 1
    fi
...
print_banner() {
    ...
    log_blue "Campaign - OPTIMIZED (128³, 5 snapshots)"
    echo "=============================================="
    echo "Athena++ binary: $ATHENA_BINARY"
    echo "Output directory: $SIMULATION_BASE"
    echo "Workers: $NUM_WORKERS"
    echo ""
    echo "OPTIMIZED specs:"
    echo "  • Resolution: 128³ (vs. 256³) → 6-10x faster"
    echo "  • Snapshots: 5 (vs. 40) → 8x less disk"
    echo "  • Outputs: TAB only (vs. VTK+TAB) → 2x less"
    echo "  • Total: 648 simulations"
    echo "  • Estimated time: ~12-15 hours on 200 cores"
    echo "  • Estimated disk: ~200 GB (vs. 3.2 TB)"
    echo ""
...
step1_generate() {
    log_info "Step 1: Generating 648 simulation configurations (OPTIMIZED)..."
...
step2_run() {
    ...
    log_info "Step 2: Running simulations with Ray (OPTIMIZED)..."
    ...
    python3 run_campaign_optimized.py \
        --phase "$phase" \
...
import subprocess
total_size = 0
for dirpath, dirnames, filenames in os.walk('$SIMULATION_BASE'):
    for filename in filenames:
        if filename.endswith('.tab') or filename.endswith('.hst'):
            total_size += os.path.getsize(os.path.join(dirpath, filename))
print(f'  Disk usage: {total_size/1e9:.1f} GB')
"
    else
...
main() {
    ...
    while [[ $# -gt 0 ]]; do
        case $1 in
            ...
            *)
                ...
                echo "Usage: bash quickstart_optimized.sh [--step STEP] [--phase PHASE] [--resume]"
    ...
    case "$step" in
        ...
        *)
            ...
            echo "Usage: bash quickstart_optimized.sh [--step STEP]"
            echo "Steps: 1 (generate), 2 (run), 3 (analyze), all (default)"
            exit 1
            ;;
    esac
    ...
    log_info "Campaign (OPTIMIZED) finished successfully!"
    echo ""
    log_info "Optimization results:"
    log_info "  • 128³ resolution: 6-10x faster than 256³"
    log_info "  • 5 snapshots: 8x less disk than 40"
    log_info "  • TAB only: 2x less disk than VTK+TAB"
    log_info "  • Total reduction: ~100x less disk"
    echo ""
    log_info "Next steps:"
