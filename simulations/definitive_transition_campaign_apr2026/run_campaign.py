...
Usage: python run_campaign.py [--phase PHASE] [--num-workers N]

Phases:
  - primary: Run primary grid (M = 1.0, 2.0, 3.0)
  - extended: Run extended grid (M = 4.0, 5.0)
...
def run_athena_simulation(sim_config, athena_binary, sim_index, total_sims):
    ...
    try:
        ...
        result = subprocess.run(
            ...
            capture_output=True,
            text=True,
            timeout=18000  # 5 hour timeout per simulation
        )

        elapsed = time.time() - start_time

        if result.returncode == 0:
            print(f"[{datetime.now().strftime('%H:%M:%S')}] [{sim_index}/{total_sims}] ✓ Completed: {sim_name} ({elapsed/3600:.2f}h)")
            return {
                'run_id': sim_name,
                'status': 'completed',
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Run Definitive 2D Transition Campaign")
    ...
    print(f"Estimated wall time: {len(manifest) * 4 * 64 / args.num_workers / 3600:.1f} hours")
    print(f"Estimated completion: {(datetime.now().timestamp() + len(manifest) * 4 * 64 / args.num_workers))}")
    print()
    ...
    while futures:
        ...
        for future in ready_futures:
            ...
            if completed % 10 == 0:
                ...
                print(f"[{completed}/{total_sims}] Progress: {completed/total_sims*100:.1f}% | Rate: {rate:.1f} sims/h | ETA: {eta/3600:.1f}h")
