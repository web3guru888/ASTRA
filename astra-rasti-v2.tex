% astra-rasti.tex — ASTRA RASTI Paper V2.0
% Original conversion from PDF text on 2026-04-04; enhanced to V2.0
...
% ======================================================================
\begin{abstract}
We present \astra (Autonomous System for Scientific Discovery in Astrophysics), an integrated framework combining numerical data analysis, causal reasoning, and physical validation within a unified, reproducible pipeline.
\astra integrates dimensional analysis, structural causal models, Bayesian evidence computation, and multi-wavelength data fusion.
We validate the framework through six controlled test cases using real observational data---Herschel filament scaling relations, Chandra Deep Field South multi-wavelength cross-matching, SDSS galaxy property analysis, Gaia stellar causal inference, and Bayesian model comparison---plus a discovery-mode demonstration on synthetic data with known ground truth.
...
Beyond these controlled tests, live deployment on archival data independently recovers Kepler's third law ($R^{2}=0.9895$ from 5\,510 exoplanets), the observational signature of dark energy from 1\,701 Type~Ia supernovae, and galaxy colour bimodality from 2\,000 SDSS galaxies---results consistent with established physics and obtained without prior domain guidance.
...
\astra analysed 5\,510 confirmed exoplanets from the NASA Exoplanet Archive.
Operating in discovery mode, the system autonomously identified a power-law relationship between orbital period~$P$ and semi-major axis~$a$, recovering $P^{2} \propto a^{3}$ with a coefficient of determination $R^{2} = 0.9895$ and an exponent within 2.3\,per\,cent of the theoretical Keplerian value.
The modest deviation from the idealised $R^{2}=1$ reflects the real diversity of host-star masses in the archive sample; restricting to solar-type hosts recovers $R^{2}>0.999$.
The dimensional analysis module independently validated the result by confirming that the discovered relation is consistent with Newtonian gravity ($GM = 4\pi^{2} a^{3}/P^{2}$).
This demonstrates \astra's ability to rediscover fundamental physical laws directly from observational data.

\subsection{Accelerating Expansion (Dark Energy Signature)}
\label{sec:live:sne}

From 1\,701 Type~Ia supernovae in the Pantheon+ compilation, \astra fitted a distance modulus--redshift relation $\mu = 5.36\,\log_{10}(z) + 43.67$.
...
Analysis of 2\,000 SDSS galaxies revealed a bimodal distribution in rest-frame $u{-}g$ colour, with GMM-fitted peaks at $u{-}g = 1.29$ (blue cloud) and $u{-}g = 1.71$ (red sequence).
...
This result is consistent with the well-established colour bimodality observed in large galaxy surveys \citep{Abazajian2009} and validates \astra's unsupervised pattern recognition on complex, multi-dimensional datasets.

\subsection{HR Diagram and the Main Sequence}
\label{sec:live:hr}

Using 4\,984 stars from Gaia Early Data Release~3, \astra recovered the main-sequence relationship $M_{G} = 3.66 \times (G_{\mathrm{BP}} - G_{\mathrm{RP}})$ with a Pearson correlation $r = 0.90$.
...
Applying the \pc and \fci causal discovery algorithms to a combined stellar--galaxy dataset, \astra correctly identified redshift as a cause of observed colour (cosmological reddening), rather than the reverse.
